...
const meshSurfaceEngine& classicBoundaryLayers::surfaceEngine() const
{
    if( !msePtr_ )
...
const meshSurfacePartitioner& classicBoundaryLayers::surfacePartitioner() const
{
...
void classicBoundaryLayers::findPatchesToBeTreatedTogether()
{
...
void classicBoundaryLayers::addLayerForPatch(const label patchLabel)
{
...
classicBoundaryLayers::classicBoundaryLayers
(
    polyMeshGen& mesh
)
:
...
void classicBoundaryLayers::addLayerForPatch(const word& patchName)
{
    if( !geometryAnalysed_ )
...
void classicBoundaryLayers::addLayerForPatches
(
    const wordList& selectedPatchNames
)
{
    if( !geometryAnalysed_ )
        findPatchesToBeTreatedTogether();
    ...
    boolList selected(boundaries.size(), false);

    label nMatched = 0;
    label nMissing = 0;

    forAll(selectedPatchNames, nameI)
    {
        bool found = false;
        ...
        if( !found )
        {
            ...
            WarningIn
            (
                "classicBoundaryLayers::addLayerForPatches"
                "(const wordList&)"
            )
                << "Cannot find selected boundary-layer patch '"
                << selectedPatchNames[nameI] << "'"
                << endl;
        }
    }

    // The classic algorithm may group neighbouring patches together.
    // Remove every non-selected patch from those groups so a wall
    // cannot pull an inlet/outlet/periodic patch into the extrusion.
    forAll(treatPatchesWithPatch_, patchI)
    {
        DynList<label>& group =
            treatPatchesWithPatch_[patchI];

        for(label i=group.size()-1; i>=0; --i)
        {
            const label groupedPatch = group[i];

            if
            (
                groupedPatch < 0
             || groupedPatch >= label(selected.size())
             || !selected[groupedPatch]
            )
            {
    ...
    // addLayerForAllPatches() interprets treatedPatch_=true as
    // "do not generate another layer here".
    treatedPatch_ = true;

    forAll(selected, patchI)
    {
        if( selected[patchI] )
            treatedPatch_[patchI] = false;
    }

    Info
        << "CLASSIC_PATCH_SELECTION:"
        << " requested=" << selectedPatchNames.size()
        << " matched=" << nMatched
        << " missing=" << nMissing
        << " totalBoundaryPatches=" << boundaries.size()
        << endl;
...
void classicBoundaryLayers::createOTopologyLayers()
{
    patchWiseLayers_ = false;
}

void classicBoundaryLayers::terminateLayersAtConcaveEdges()
{
    terminateLayersAtConcaveEdges_ = true;
}

void classicBoundaryLayers::activate2DMode()
{
...
void classicBoundaryLayers::addLayerForAllPatches()
{
    if( !geometryAnalysed_ )
        findPatchesToBeTreatedTogether();
